function [preimages,images,averages,mask] = OMimprocess(fname,im,rect,num_images,cropchoice,mask,sfilt,sfiltsize,inversion,tfilt,removef,camopt,sfiltsigma,pbefore,pafter,n,tstack)
%funtion for reading in and processimg all images in the tif stack/ rhs
[rows,cols] = size(im);
[~,~,remain] = fileparts(fname);
fileisrsh=0;
...
for j  = 1:num_images;  
    ...
    croptime=croptime+toc;
    tic
    if inversion == 0
       A=imcomplement(A);
    end
    invertime=invertime+toc;
    preimages(:,:,j)=A;
    tic
    A = imfilter(A, h);
    filttime=filttime+toc;
    tic
    A = uint16(A);
    convtime=convtime+toc;
    mask=uint16(mask);
    A=uint16(A);
    B = A.*mask;
    images(:,:,j) = B;
